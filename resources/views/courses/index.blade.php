<x-app-layout>
    <x-slot name="header">
        <div class="flex justify-between items-center">
            Courses
            <x-action-button variant="primary" icon="add" :href="route('courses.create')">
                Add New Course
            </x-action-button>
        </div>
    </x-slot>
    ...
    <div x-data="{ activeModal: null }">
                        ...
                        @foreach($courses as $course)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                    <x-action-button variant="secondary" icon="view" :href="route('courses.show', $course)" class="!px-3 !py-1">
                                        View
                                    </x-action-button>
                                    <x-action-button variant="primary" icon="edit" :href="route('courses.edit', $course)" class="!px-3 !py-1">
                                        Edit
                                    </x-action-button>
                                    <x-action-button variant="danger" icon="delete" @click="activeModal = {{ $course->id }}" class="!px-3 !py-1">
                                        Delete
                                    </x-action-button>
                                </td>
                            </tr>
        ...
        @foreach($courses as $course)
            <div x-show="activeModal === {{ $course->id }}"
                 style="display: none;"
                    ...
                    <div x-show="activeModal === {{ $course->id }}"
                        ...
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                ...
                                <div class="mt-2">
                                    <p class="text-sm text-gray-500">Are you sure you want to delete '{{ $course->name }}'? This action cannot be undone.</p>
                                </div>
                            </div>
                        </div>
